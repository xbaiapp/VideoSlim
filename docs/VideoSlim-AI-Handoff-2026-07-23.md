...
>
> **生成日期：** 2026-07-23
> **修订：** r6（2026-07-23）— r5规划经所有者批准入库后，完成C1a最小实现、唯一修订、自动化、ARM64 APK和静态核验；其余C轨与M4-B/M4-C状态不变。规划演进与实现记录见§16。
...
**C1a低收益/可能变大提示已进入真机验收。** C1b/C2/C3/M4-B/M4-C仍未授权。当前`1.7.0+23`候选包含此前`1.6.1+22`全部生产行为，但既有真机验收债务不会因新构建而自动转为PASS：
...
| 压缩策略修正 C 轨（D1/C1a/C1b/C2/C3） | `C1a CANDIDATE READY — DEVICE ACCEPTANCE PENDING` | C1a唯一修订、自动化和APK静态核验完成；D1待指定日志，C1b/C2/C3仍未授权 |
...
1. 完成C1a与§6挂起的P0/P1真机验收；
2. 按指定任务读取既有F19记录执行D1零代码诊断；仅当记录缺失时才另行授权一条日志的最小修；
...
#### C1 源感知提示（C1a，候选待真机）与建议性目标（C1b，未授权）（r5 拆分）
...
6. `docs/c1a-low-savings-device-acceptance.md`
7. `docs/VideoSlim PRD.md` — 产品和架构权威contract
...
12. `docs/m3-completion-report.md`
13. `docs/known-debt.md`
14. `AGENTS.md` — 审批、复审预算和blocker规则
...
**当前最安全的继续方式：覆盖安装`1.7.0+23`并完成C1a与既有清单；没有新的可复现缺陷前，不再改生产代码。**
