...
>
> **生成日期：** 2026-07-23
> **修订：** r12（2026-07-23）— C2/F21只读编码器能力诊断已在目标`1.9.0+25`工作树实现并通过冻结前自动化；源码冻结、唯一复审、APK核验和真机清单仍待完成。规划演进与实现记录见§16。
...
> **规范分支：** `m4a/crop`
> **当前可执行候选源码：** `9351e75bcc43c71a6e7caf03093fe27b0072b061`
> **当前候选版本：** `1.8.0+24`
> **当前候选状态：** M4-B/F8 `ACCEPTED — private scope`（所有者报告成功；详细矩阵未提供）
> **C2工作树目标：** `1.9.0+25`，`IMPLEMENTED — SOURCE FREEZE PENDING`
> **项目阶段：** 私有自用；当前只允许收敛C2/F21冻结、复审、构建与验收
...
**C1a已实现，但项目所有者明确跳过其真机验收；未执行矩阵仍是PENDING，不能写PASS。** D1已完成。M4-B/F8首个冻结SHA的一次双路复审为一路PASS、一路BLOCKERS；接受的阻断已在唯一纠正SHA `9351e75...`中修复。项目所有者随后报告`1.8.0+24`测试成功并接受private scope；因未提供设备/素材/PTS/日志，逐项矩阵仍保持PENDING。C2/F21只读能力诊断工作树现已实现并通过Flutter`257/257`与Android JVM`350/350`冻结前门禁，但尚未冻结、复审或形成APK候选；C1b/C3/M4-C仍未授权。所有既有真机债务不会因新候选而自动转为PASS：
...
M4-B已接受private scope；当前只授权C2/F21只读能力诊断。M4-C、M5打磨、C1b/C3、hardening、refactor、migration均**没有自动开工权限**。
...
| 压缩策略修正 C 轨（D1/C1a/C1b/C2/C3） | `C2/F21 IMPLEMENTED — SOURCE FREEZE PENDING` | C1a未真机测试、不记PASS；D1确认Pixel HEVC运行期明显过冲；C2只读能力页、严格wire和Android reader已通过冻结前门禁，C1b/C3未授权 |
...
源码：9351e75bcc43c71a6e7caf03093fe27b0072b061
源码 tree：c9807183c59bbce7fdf47ac79fac2fde24c179ed
ABI：arm64-v8a only
...
该APK只用于项目所有者私有真机验收，不是production-signed商店包。旧`1.6.0+21`因日志复制缺陷已被隔离；`1.6.1+22`和`1.7.0+23`保留为历史候选证据，不再作为当前包分发。
...
2. C2编码能力探测（当前唯一代码项；小型诊断改动，不改转码行为）；
...
### 7.1 C 轨：压缩策略修正（C1a已实现/测试跳过；D1完成；C2工作树已实现）
...
实现状态（r12）：目标`1.9.0+25`工作树已完成严格Dart report、`getEncoderCapabilities({})`平台通道、Android只读reader、F19入口、独立页面、刷新generation隔离、空/错误/单项失败状态与确定性复制文本；Flutter`257/257`、Android JVM`350/350`冻结前自动化通过。exact source SHA、唯一复审、release构建/APK核验与真机能力清单尚未完成。
...
**当前最安全的继续方式：只使用`1.8.0+24 / 9351e75...`执行M4-B真机验收并记录真实证据；不得继续修改生产代码或并行扩张到M4-C及其他项目。**
