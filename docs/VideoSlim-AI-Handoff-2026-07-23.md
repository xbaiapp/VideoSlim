...
>
> **生成日期：** 2026-07-23
> **修订：** r14（2026-07-23）— Pixel 10 Pro / API 37 C2/F21能力报告已回传并归档：9 entries、四种固定MIME、0 query errors；硬件AVC/HEVC声明QP bounds，硬件CQ均不可用，硬件AV1存在但无QP bounds。项目所有者接受决定与未报告交互项仍PENDING。规划演进与实现记录见§16。
...
> **规范分支：** `m4a/crop`
> **当前可执行候选源码：** `11f169ca9f30b2f05eeeec777dbaaaf71a01f7ff`
> **当前候选版本：** `1.9.0+25`
> **当前候选状态：** C2/F21 `DEVICE REPORT CAPTURED — OWNER DISPOSITION PENDING`
> **当前已接受基线：** M4-B/F8 `1.8.0+24 / 9351e75...`（所有者报告成功；详细矩阵未提供）
> **项目阶段：** 私有自用；C2报告已归档，等待所有者接受/拒绝或补测剩余交互项；其他代码项仍需独立授权
...
**C1a已实现，但项目所有者明确跳过其真机验收；未执行矩阵仍是PENDING，不能写PASS。** D1已完成。M4-B/F8首个冻结SHA的一次双路复审为一路PASS、一路BLOCKERS；接受的阻断已在唯一纠正SHA `9351e75...`中修复。项目所有者随后报告`1.8.0+24`测试成功并接受private scope；因未提供完整设备/素材/PTS/日志矩阵，未报告项仍保持PENDING。C2/F21首个SHA `85e2497...`的唯一双路复审均超时无裁决，controller exact gate发现API 29 lint blocker；唯一纠正SHA `11f169c...`通过Flutter`257/257`、Android JVM`350/350`、完整lint/build与APK静态核验，但按预算未复审纠正SHA。Pixel 10 Pro / API 37能力报告随后回传：9 entries、四种固定MIME和0 query errors；C2项目所有者接受决定与部分交互/副作用项仍PENDING，C1b/C3/M4-C仍未授权。所有既有真机债务不会因新候选而自动转为PASS：
...
M4-B已接受private scope；C2/F21代码与私有候选已完成。M4-C、M5打磨、C1b/C3、hardening、refactor、migration均**没有自动开工权限**。
...
源码：11f169ca9f30b2f05eeeec777dbaaaf71a01f7ff
...
该APK只用于项目所有者私有真机能力验收，不是production-signed商店包。M4-B `1.8.0+24`仍是已接受私有基线；旧候选与各自证据继续保留，不被C2结果改写。
...
### 6.2 当前唯一优先队列：M4-B真机验收

项目所有者已跳过C1a真机矩阵；C1a仍为`DEVICE TEST WAIVED/NOT RUN`，不得重开或改写为PASS。M4-B实现、唯一复审/修订预算、纠正SHA门禁和APK静态核验均已完成，当前只执行设备验收。

#### P0：覆盖安装并执行M4-B矩阵

1. 使用相同Debug certificate覆盖安装`1.8.0+24`，不要先卸载，以保留应用数据和旧F19日志；
2. 执行`docs/m4-b-device-acceptance.md`，先验证无trim回归，再验证掐头、去尾、两端与恰好1秒；
3. 继续验证trim与crop/720p、H.264/HEVC、audio copy/reencode/remove的组合，以及普通retry和兼容Decoder retry端点保持；
4. 执行后台、锁屏、Activity重建、进程恢复、取消、MediaStore/SAF与源文件安全检查；
5. 保留选定端点、输出PTS、主观音画同步、截图/录屏和脱敏F19日志；自动化或静态APK结果不能填入设备PASS栏。
...
完成M4-B及仍需执行的既有矩阵证据后，项目所有者对对应候选明确选择：

- `ACCEPTED — private scope`；或
- `REJECTED — blocker found`；或
- 只授权一个可复现问题的局部修订。

未获得该决定前，不得把M4-A、M4-B或metadata增强写为ACCEPTED。
...
2. C2编码能力探测（私有候选已完成，当前等待真机能力清单；不改转码行为）；
...
**当前最安全的继续方式：安装`1.9.0+25 / 11f169c...`私有候选，从F19打开“编码器能力”，复制并回传完整真机能力清单；在新授权前不得继续修改生产代码或并行扩张到C1b/C3/M4-C及其他项目。**
