...
| 压缩策略修正 C 轨（D1/C1a/C1b/C2/C3） | `PLANNED — NOT AUTHORIZED` | r2 新增规划（§7.1）；D1 诊断先行 |
...
1. 完成 §6 挂起的 P0/P1 真机验收（不变，仍是唯一进行中的队列）；
2. D1 码率下发诊断（第一步为读取既有 F19 记录，零代码；仅当记录缺失时才含一条日志的最小修）；
3. C1a 低收益/可能变大提示（小，纯 Dart）；C1b 建议目标为后置条件项（依赖显式码率来源契约、D1 结论与系数校准，见 §7.1），与 C3 决策窗口一并评估，不单列排期；
4. M4-B 单段时间裁剪（中）；
...
### 7.1 C 轨：压缩策略修正（r2 新增，`PLANNED — NOT AUTHORIZED`）
...
#### C1 源感知提示（C1a）与建议性目标（C1b）（推荐项，授权后实施；r5 拆分）
