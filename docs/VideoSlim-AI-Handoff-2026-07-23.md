...
>
> **生成日期：** 2026-07-24
> **修订：** r16（2026-07-24）— 冻结`1.9.2+27 / 7948f9f...` AAC lossless-copy cadence修正、App端真机成功补证、自动software decoder fallback自然failure contract处置和workflow-only CI修正边界。原C2/F21能力报告保持冻结。规划演进与实现记录见§16。
...
> **规范分支：** `m4a/crop`
> **当前可执行候选源码：** `7948f9fdbfc839b26d6055ca83ddf8449df42de5`
> **当前候选版本：** `1.9.2+27`
> **当前候选状态：** M3 cadence修正`APP-LEVEL DEVICE LOG PASS — SUBJECTIVE PLAYBACK PENDING`；自动software decoder fallback`NATURAL DEVICE FALLBACK CONTRACT OBSERVED — OWNER DISPOSITION PENDING`
> **当前已接受基线：** M4-B/F8 `1.8.0+24 / 9351e75...`（所有者报告成功；详细矩阵未提供）
> **项目阶段：** 私有自用；workflow-only CI已全绿，完成文档证据冻结后设备主观/扩展矩阵继续PENDING；C2报告保持独立冻结，其他代码项仍需明确授权
...
- 一个连续半开时间区间，与crop、缩放、codec、码率和音频模式在同一次Media3导出中完成；
- 只保留可靠来源拍摄时间/GPS，发布前同时核验“应有”和“应无”；
- 默认 MediaStore / 自定义 SAF 输出、真实文件名与位置、打开与分享；
- F19 本地调试日志，超长日志只复制最近128 KiB，完整日志走文件分享。
- 首次硬件视频attempt若结构化解码失败，在同一用户task和同一前台service内无确认地从头使用software decoder重试一次；新任务仍默认硬件，不循环、不建立永久黑名单。

**C1a已实现，但项目所有者明确跳过其真机验收；未执行矩阵仍是PENDING，不能写PASS。** D1已完成。M4-B/F8已由项目所有者报告成功并按private scope接受。C2/F21的`1.9.0+25 / 11f169c...`与Pixel 10 Pro/API 37能力报告保持冻结。自动fallback的真实device contract已自然观察：硬件Decoder约79%结构化失败后，同task/同service切software并回到0%，后续独立Encoder失败无循环或残缺发布。`1.9.2+27`只修正M3 AAC copy cadence；真实`23,812 µs`素材已单次发布成功，主观播放PENDING，完整frame gap仍由自动化拒绝。C1b/C3/M4-C仍未授权。所有既有真机债务不会因新候选而自动转为PASS：

1. 复测约1 MiB日志复制不再触发Android Binder异常；
2. 补齐M4-A裁剪真机矩阵；
3. 补齐真实iPhone/Pixel来源的时间/GPS、MediaStore `DATE_TAKEN`、图库排序和SAF矩阵；
4. 补齐自动fallback最终成功发布/播放、通知截图、取消和后台恢复矩阵；
5. 确认`1.9.2+27`新M4A实际播放与声音；
6. 由项目所有者决定接受、拒绝或授权下一项明确范围。

M4-B已接受private scope；C2/F21和自动fallback代码候选均已完成。M4-C、M5打磨、C1b/C3、hardening、refactor、migration均**没有自动开工权限**。

r2历史上只授权写入规划；r7中项目所有者单独授权§7.2 M4-B，r11在其接受后单独授权§7.1 C2。§7.3 M4-C及C1b/C3仍为`PLANNED — NOT AUTHORIZED`。
...
| M3 AAC copy cadence修正 | `APP-LEVEL DEVICE LOG PASS — SUBJECTIVE PLAYBACK PENDING` | `1.9.2+27 / 7948f9f...`已在同类Pixel素材单次发布成功；实际播放/声音仍待确认 |
...
| M4-B时间裁剪 | `ACCEPTED — private scope` | `1.8.0+24`由所有者报告测试成功；详细逐项矩阵未提供，不能外推多设备保证 |
| M4-C多段时间编辑 | `PLANNED — NOT AUTHORIZED` | M4-B依赖已满足，但仍需独立授权 |
| 压缩策略修正 C 轨（D1/C1a/C1b/C2/C3） | `C2/F21 DEVICE REPORT CAPTURED — OWNER DISPOSITION PENDING` | API 37报告9 entries、四种MIME、0 query errors；硬件AVC/HEVC有QP bounds、硬件CQ均不可用、硬件AV1存在但无QP bounds；纠正SHA无独立review PASS，C1b/C3未授权 |
| 一次性自动软件解码重试 | `NATURAL DEVICE FALLBACK CONTRACT OBSERVED — OWNER DISPOSITION PENDING` | 真实硬件Decoder failure已进入同task一次software attempt；独立Encoder失败正确终止，无循环、残缺发布或recovery泄漏 |
...
源码：7948f9fdbfc839b26d6055ca83ddf8449df42de5
...
该APK只用于项目所有者私有真机验收，不是production-signed商店包。M4-B `1.8.0+24`仍是已接受私有基线；原C2 `1.9.0+25 / 11f169c...`与各自证据继续保留，不被新候选改写。
...
6. 新视频任务始终先硬件decoder；只有首次结构化`VIDEO_DECODING_FAILED`可在同task/同service内无确认地software retry一次，不循环、不永久降级。
7. 裁剪、缩放、压缩必须一次转码完成，不创建中间视频后再二次有损编码。
8. 拍摄metadata只允许可靠时间和GPS；不复制完整XMP/mdta、设备型号、软件版本或厂商私有atom。
9. metadata字段缺失时保持缺失；不得用处理时间、文件mtime、当前位置或文件名伪造。
10. 不增加隐私模式、音频继承视频metadata、第二次remux、自定义MP4 writer或Media3升级，除非项目所有者重新批准范围。
11. 自动化、模拟器和静态review不能替代真机证据。
...
### 5.7 一次性自动software decoder fallback

- 仅首次hardware视频attempt的结构化`VIDEO_DECODING_FAILED`有资格；encoder/audio/storage/publication/general failure、取消与forced finish不触发；
- 同一用户task、同一`ProcessingService`、同一publication identity，使用新的内部engine route；Flutter不提交第二任务；
- 原request精确复用，只把decoder mode改为software；下一次新任务仍先hardware；
- 旧Transformer/codec、temp、recovery结束后才启动新attempt；route、publication、registry、recovery任一转移失败均fail closed；
- registry、Flutter和通知以显式`automaticSoftwareDecoderRetry`重置到`preparing/0%`；延迟硬件snapshot不能覆盖attempt boundary；
- software attempt失败后普通terminal，不循环；旧route迟到event/publish被拒绝；
- exact门禁、APK和审查边界见`docs/automatic-software-decoder-retry-completion-report.md`，device path见对应acceptance清单。
...
| M4-B实现与唯一复审 | 首个SHA一路PASS、一路BLOCKERS；接受的恢复阻断已在唯一纠正修订中处置 |
| M4-B纠正SHA自动化 | `9351e75...`完成Flutter `244/244`、Android JVM `346/346`、debug/release lint和debug assemble |
| 构建`1.8.0+24` | ARM64 release、ZIP、16 KiB zipalign、v2签名、证书连续性、身份/权限/秘密扫描通过 |
| 自动decoder fallback实现 | 同task/同service、精确request、一次性eligibility、route/publication/registry/recovery转移、无确认UI与通知已完成 |
| 唯一review与修订 | `11799b0...`独立review超时无verdict；controller两项attempt-boundary finding已在唯一修订`8f4e970...`关闭 |
| 构建`1.9.1+26` | Flutter`259/259`、Android JVM`359/359`、完整lint/assemble、ARM64 release和APK静态核验通过 |
| 自动fallback真机contract | Pixel自然硬件Decoder failure约79%后，同task/同service切software、回到0%、实际软件Decoder；独立Encoder失败无循环、残缺发布或recovery泄漏 |
| M3 cadence修正 | 真实`23,812 µs`fixture RED→GREEN；初版review blocker由`1,000 µs`绝对继承上限和完整frame-gap反例测试关闭 |
| 构建`1.9.2+27` | focused`22/22`、Flutter`259/259`、Android JVM`365/365`、完整lint/build、ARM64 APK与静态核验通过；Pixel同类AAC任务单次发布成功 |

### 6.2 当前唯一优先队列：证据冻结与设备剩余项

生产代码、唯一review/revision预算、最终门禁、APK核验和workflow-only CI重跑均已完成。当前不再修改生产代码，只同步文档，并把不能由日志证明的设备项保留为PENDING。

#### P0：覆盖安装并验证当前候选

1. 当前`1.9.2+27`继续使用相同Debug certificate覆盖安装，禁止先卸载；
2. M3同类AAC任务的App端成功日志已归档，等待项目所有者确认实际播放、声音与时长；
3. 自动fallback真实failure contract已归档，剩余最终成功发布/播放、通知截图、software取消和后台恢复项保持PENDING；
4. API 35 x86_64 instrumentation workflow-only重跑已`3/3 PASS`；不得把模拟器结果替代物理设备；
5. 保留完整F19日志，但文档不得包含用户URI、任务ID、媒体名或秘密值。

#### P1：既有PENDING矩阵继续独立保留

- M4-A rotation/crop矩阵、metadata/`DATE_TAKEN`/SAF来源矩阵和超长日志设备复测不会因本候选自动变成PASS；
- C2刷新、返回、重启与无任务/通知/文件副作用项保持原状态；
- C1b/C3/M4-C仍未授权，不与当前验收混跑。

#### P2：项目所有者决策

项目所有者需分别处置：`1.9.2+27` M3 cadence修正、自动fallback contract、C2能力报告，以及下一生产路线。未获得明确决定前不得把PENDING项写成ACCEPTED，也不得把“继续”泛化为任选C1b/C3/M4-C中的一个。
...
1. M4-B单段时间裁剪（已接受private scope；详细矩阵未提供）；
2. 一次性自动software decoder fallback（实现完成，真实contract已观察，所有者处置与剩余矩阵PENDING）；
3. C2编码能力探测（报告已归档，接受处置与未报告交互项独立保留）；
4. C1b建议目标为后置条件项（依赖显式码率来源契约、D1结论与系数校准，见§7.1），与C3决策窗口一并评估，不单列排期；
5. C3按C2证据四选一，含"都不做"（中）；
6. M4-C多段时间编辑（中大，依赖独立授权）；
7. M5原范围顺延，其中F13建议提级（见7.4 r2备注）。

每项独立授权、独立验收；同一时间不并行两条编码任务。r4 修正：D1 第一步（读既有日志）零代码、可立即执行，无需授权流程；C2 虽只查询能力、不改转码行为，但仍是完整的小型代码任务（候选、测试、复审、真机），按独立小候选走，不与 D1 捆绑。

### 7.1 C 轨：压缩策略修正（C1a已实现/测试跳过；D1完成；C2候选等待设备报告）
...
项目所有者于2026-07-23报告M4-B测试成功并要求继续下一步，按§7.0顺序授权本节作为唯一代码项；该授权不包含C1b、C3或任何转码策略变化。

实现状态（r14）：纠正候选`1.9.0+25 / 11f169c...`已完成严格Dart report、`getEncoderCapabilities({})`平台通道、Android只读reader、F19入口、独立页面、刷新generation隔离、空/错误/单项失败状态与确定性复制文本；Flutter`257/257`、Android JVM`350/350`、debug/release lint、debug assemble、ARM64 release build与APK静态核验通过。唯一双路复审绑定首个SHA `85e2497...`且均超时无裁决；controller发现的API 29 lint blocker已在唯一纠正修订中修复，按预算未复审纠正SHA。Pixel 10 Pro / API 37报告已归档：9 entries、7个唯一canonical实现、2个alias、四种固定MIME和0 query errors；项目所有者接受决定、刷新及部分只读副作用检查仍PENDING。
...
### 7.2 M4-B：单段时间裁剪 F8（r2补全计划；r11 `ACCEPTED — private scope`）
...
Android JVM tests                   346/346 PASS
Android debug/release lint          PASS
Android debug assemble              PASS
ARM64 Flutter release build         PASS
APK ZIP/16KiB zipalign/v2 signature PASS
package/version/ABI/permission scan PASS
common credential-pattern scan      PASS
M4-B first review 9c9ca887...        PASS / BLOCKERS
M4-B revision 9351e75...             blocker已处置；按预算未二次review
APK SHA-256                         ac85d84e0a69185b3e73180737918fb98326f3899db9264991c0a9c681351567
```

上一C1a候选`7c49e57...`的Flutter `227/227`、Android JVM `341/341`和APK静态核验继续作为历史证据；其首版两路FAIL中的三项IMPORTANT已在唯一修订中处置，且设备矩阵由项目所有者跳过。不得把这些历史结果冒充当前M4-B设备证据。
...
8. 当前没有MP3、多音轨选择、多段时间线、音量/降噪、混音、采样率选择、隐私模式、完整metadata复制、iOS或商店发布。
...
4. `README.md`
5. `docs/m4-b-completion-report.md`
6. `docs/m4-b-exact-sha-review-disposition.md`
7. `docs/m4-b-device-acceptance.md`
8. `docs/d1-bitrate-diagnosis-2026-07-23.md`
9. `docs/c1a-low-savings-completion-report.md`
10. `docs/c1a-low-savings-device-acceptance.md`
11. `docs/VideoSlim PRD.md` — 产品和架构权威contract
12. `docs/capture-metadata-completion-report.md`
13. `docs/capture-metadata-device-acceptance.md`
14. `docs/m4-a-completion-report.md`
15. `docs/m4-device-acceptance.md`
16. `docs/m3-completion-report.md`
17. `docs/known-debt.md`
18. `AGENTS.md` — 审批、复审预算和blocker规则

旧完成报告中的历史SHA只证明对应历史候选；当前M4-B APK源码以`9351e75bcc43c71a6e7caf03093fe27b0072b061`为准。后续docs-only HEAD只更新候选证据，不是APK构建源码。
...
3. 当前只实施C2/F21；不启动C1b/C3、M4-C、hardening、refactor或migration。
...
1. C2只读平台契约、F19页面、失败隔离和能力清单格式是否与§7.1一致；
2. RED→GREEN测试是否证明不configure codec、不建媒体任务、不改现有转码路径；
3. C2候选必须独立版本、一次实现/一次修订/一次复审，并保留真机能力输出为PENDING直到所有者运行；
4. 两个既有真机矩阵（M4-A十项、时间/GPS来源）继续保持原状态；C1a已由所有者跳过，不得要求补测或写PASS；
5. C3为四选一（含“都不做”），必须等C2真机证据，不得提前实现；软件CRF已记录为不做（§7.1），除非所有者按复议条件重开。

如果用户只说“继续”，默认从已授权C2实现的实际仓库状态继续，不自动进入C1b/C3/M4-C。
...
**当前最安全的继续方式：保持可执行身份绑定`1.9.2+27 / 7948f9f...`；workflow-only CI已全绿，完成文档冻结后让M3实际播放/声音和自动fallback剩余设备矩阵保持PENDING。下一生产路线已到明确决策点，在新授权前不得继续修改生产代码或并行扩张到C1b/C3/M4-C及其他项目。**
...
- **r9（2026-07-23，M4-B唯一复审与纠正修订）**：
  1. 首个冻结SHA `9c9ca887ec4a8a26a6eb892077f88d333eb2a0da`完成唯一双路复审；首批两个广泛审查均超时且无裁决，同一未变SHA的限时替代裁决为一路PASS、一路BLOCKERS；
  2. 接受的阻断为：终态`INVALID_TRIM`快照在来源metadata缩短时会在消费终态前重新校验失败，外层进入所有权不确定锁定，用户无法重新编辑或移除；
  3. 唯一纠正修订保留快照端点并显式记录`trimNeedsRepair`；新保存继续fail closed，恢复编辑器不夹取、不自动保存，而以全时长中性范围和明确提示要求用户重新选择或返回移除；
  4. 三条回归先RED后GREEN，Dart format、Flutter analyze与Flutter`244/244`已通过；纠正源码仍待冻结后完整exact-SHA门禁和APK独立核验；
  5. 按每任务一轮复审预算不追加第二轮，也不得把旧SHA的混合裁决写成纠正SHA的PASS；完整处置见`docs/m4-b-exact-sha-review-disposition.md`；
  6. 版本仍为`1.8.0+24`，因为旧SHA尚未构建或分发APK；设备证据仍为`NOT RUN`，C1b/C2/C3/M4-C继续未授权。

- **r10（2026-07-23，M4-B纠正候选与APK核验）**：
  1. 唯一纠正源码冻结为`9351e75bcc43c71a6e7caf03093fe27b0072b061`，tree为`c9807183c59bbce7fdf47ac79fac2fde24c179ed`；
  2. exact-SHA门禁通过：Dart format、Flutter analyze、Flutter`244/244`、Android JVM`346/346`、debug/release lint和debug assemble；
  3. `1.8.0+24` ARM64内部APK完成ZIP、16 KiB zipalign、v2签名、证书连续性、package/version/SDK/仅ARM64 ABI、权限和秘密扫描；SHA-256为`ac85d84e0a69185b3e73180737918fb98326f3899db9264991c0a9c681351567`；
  4. 旧SHA的一路PASS/一路BLOCKERS只属于旧SHA；按预算不追加第二轮，不把它写成纠正SHA的独立PASS；
  5. M4-B晋级`CANDIDATE READY — DEVICE ACCEPTANCE PENDING`，设备状态仍为`NOT RUN — NO DEVICE EVIDENCE`；C1b/C2/C3/M4-C继续未授权。

- **r11（2026-07-23，M4-B私有接受与C2授权）**：
  1. 项目所有者通过Telegram报告`1.8.0+24`测试成功并要求继续下一步，M4-B记为`ACCEPTED — private scope`；
  2. 因未提供设备型号、素材、PTS、截图或逐项日志，`docs/m4-b-device-acceptance.md`未报告的矩阵行继续保持PENDING，不补写PASS或外推多设备保证；
  3. 按§7.0既定顺序，C2/F21本机编码器能力诊断获得单独开工授权，成为唯一代码项；
  4. C2只允许F19只读页面/平台通道和capabilities查询，不configure codec、不创建任务、不改变VBR或任何转码行为；C1b/C3/M4-C继续未授权。

- **r12（2026-07-23，C2实现与冻结前门禁）**：
  1. 固定查询AVC、HEVC、AV1和VP9 encoder；API 29前的平台软硬件字段、API 31前的QP bounds显式为unknown，不用名称启发式冒充平台结论；
  2. 单个codec/mime能力查询失败只生成`CAPABILITY_QUERY_FAILED` entry；Dart严格拒绝未知字段、非整数wire值和不一致API语义；
  3. F19只读页面支持加载、空态、错误、重试、刷新旧响应隔离和确定性清单复制；查询复用既有进程级有界本地I/O dispatcher；
  4. `TranscodeEngine`、`ProcessingService`、publication、recovery和manifest均未修改，reader不创建/configure/start codec；
  5. 冻结前门禁通过Dart format、Flutter analyze、Flutter`257/257`与Android JVM`350/350`；源码冻结、唯一复审、APK核验和真机能力清单仍待完成。

- **r13（2026-07-23，C2纠正候选与APK核验）**：
  1. 首个冻结SHA `85e249726e36325e9532e641f296854b8c8d1eb2`的一次双路exact-SHA复审均在600秒超时且没有最终裁决，按`NO VERDICT`处理；
  2. controller exact Android gate在同一SHA发现五个API 29 `NewApi` lint错误，旧SHA被否决；唯一纠正SHA `11f169ca9f30b2f05eeeec777dbaaaf71a01f7ff`改用直接SDK guard与`@RequiresApi(Q)` helper；
  3. 纠正SHA exact门禁通过Dart format、Flutter analyze、Flutter`257/257`、Android JVM`350/350`、debug/release lint和debug assemble；
  4. `1.9.0+25` ARM64内部APK完成ZIP、16 KiB zipalign、v2签名、证书连续性、package/version/SDK/仅ARM64 ABI、权限和秘密扫描；SHA-256为`fe9e0e70b90dd5ae3bd6aace327b3a636b365a112689cd7a1f994927ccb6b2ed`；
  5. 按每任务一轮复审预算不追加第二轮，不得把旧SHA的超时复审写成纠正SHA独立PASS；C2状态为`CANDIDATE READY — DEVICE CAPABILITY REPORT PENDING`，C1b/C3/M4-C继续未授权。

- **r14（2026-07-23，C2 API 37能力报告）**：
  1. 项目所有者从Pixel 10 Pro / Android 17 API 37回传完整确定性报告：9 entries、四种固定MIME、0 query errors；复制回传核心路径已获得设备证据；
  2. 9个entry对应7个唯一canonical实现和2个OMX alias；硬件/vendor实现为AVC、HEVC和AV1，VP9只有软件实现/alias；
  3. 硬件AVC/HEVC声明`QP bounds=true`，硬件AV1为false；三个硬件encoder均`CQ=false`，所有entry均声明VBR/CBR；
  4. 硬件AVC/HEVC的声明bitrate lower bound均为1 bps，因此D1低目标运行期过冲不能解释为平台声明的最低码率夹高；
  5. 状态更新为`DEVICE REPORT CAPTURED — OWNER DISPOSITION PENDING`。刷新、返回后设置不变、查询无任务/通知/文件副作用、重启重查和并发普通压缩未获独立设备证据；C1b/C3/M4-C继续未授权。

- **r15（2026-07-24，一次性自动software decoder fallback）**：
  1. 相同素材、crop/trim和参数后来再次硬件成功，因此不建立永久黑名单；每个新任务仍先hardware，仅首次结构化`VIDEO_DECODING_FAILED`自动software retry一次；
  2. fallback位于同一`ProcessingService`和用户task内；原request只改decoder mode，route、publication、registry、recovery转移，旧route迟到event/publish拒绝；
  3. Flutter不第二次提交；registry、UI和通知使用显式flag重置为`preparing/0%`，Activity恢复保持同一software attempt；
  4. 首版`11799b0...`的唯一独立review在600秒超时且无verdict；controller发现的延迟硬件snapshot边界和registry fail-open已在唯一修订`8f4e970...`关闭，不宣称纠正SHA有独立review PASS；
  5. `1.9.1+26`通过Flutter`259/259`、Android JVM`359/359`、完整lint/assemble、ARM64 release和APK静态核验；APK SHA-256为`8765c79b1afe9cfe5b451359766646b4be089d12820e3478a05ba636f24272f3`；
  6. 状态为`PRIVATE INTERNAL CANDIDATE — DEVICE ACCEPTANCE PENDING`；原C2候选及API 37证据保持冻结，C1b/C3/M4-C继续未授权。

- **r16（2026-07-24，M3 cadence修正、真机证据与CI边界）**：
  1. 真实Pixel AAC-LC 48 kHz素材在`805 samples / 206,177 payload bytes`均一致时，旧严格门限仍误杀`23,812 µs` cadence；只为lossless-copy增加source继承上下文；
  2. 初版`fe0e04d...`一次双路审查发现source/output共同完整frame gap可错误放行；唯一修订`7948f9f...`把output最大delta约束为`min(source max + 2,000 µs, 普通严格上限 + 1,000 µs)`并加入对应RED→GREEN反例；
  3. `1.9.2+27`通过focused`22/22`、Flutter`259/259`、Android JVM`365/365`、完整lint/build与APK静态核验；APK SHA-256为`7a180bc7ce8f89439c7e31f1fcb4c019d1be45d4d356c436698aad43bbcb6f18`；
  4. Pixel同类AAC任务已正确通过`23,812 µs`、单次SAF publication和清理；日志不能证明人耳播放/声音，故主观项PENDING；
  5. 旧日志已自然观察自动fallback真实contract；software attempt最终死于独立硬件Encoder错误，不是software Decoder失败，也没有第二fallback；
  6. `68aaa77...`只修复GitHub Actions的Android command-line tools配置；远端run`30107559081`全绿且API 35 x86_64 instrumentation `3/3 PASS`，但不取代`7948f9f...`可执行APK身份；C1b/C3/M4-C继续未授权。
