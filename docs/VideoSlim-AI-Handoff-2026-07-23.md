...
>
> **生成日期：** 2026-07-23
> **修订：** r9（2026-07-23）— M4-B首个冻结SHA的一次双路复审发现`INVALID_TRIM`恢复阻断；唯一纠正修订已通过Flutter `244/244`并等待冻结后的完整门禁与候选构建。规划演进与实现记录见§16。
...
> **当前候选版本：** `1.7.0+23`
> **M4-B目标版本：** `1.8.0+24`（纠正源码尚未冻结；当前不是候选）
...
**C1a已实现，但项目所有者明确跳过其真机验收；未执行矩阵仍是PENDING，不能写PASS。** D1已完成；M4-B/F8首个冻结SHA的一次双路复审为一路PASS、一路BLOCKERS，接受的`INVALID_TRIM`恢复锁定问题已进入唯一纠正修订。按预算不追加第二轮复审；纠正源码门禁和APK静态核验完成前不得称为候选。C1b/C2/C3/M4-C仍未授权。当前`1.7.0+23`候选的既有真机债务不会因继续开发而自动转为PASS：
...
| M4-B时间裁剪 | `CORRECTIVE REVISION — EXACT-SHA GATES PENDING` | 唯一双路复审发现一项恢复阻断；唯一纠正修订已通过Flutter `244/244`，多段/跨文件不在范围，候选与真机证据尚未完成 |
...
### 7.2 M4-B：单段时间裁剪 F8（r2补全计划；r9 `CORRECTIVE REVISION — EXACT-SHA GATES PENDING`）
