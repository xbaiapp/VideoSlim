...
>
> **生成日期：** 2026-07-23
> **修订：** r10（2026-07-23）— M4-B纠正SHA已通过完整自动化与APK静态核验；`1.8.0+24` ARM64内部候选等待真机验收。规划演进与实现记录见§16。
...
> **当前候选版本：** `1.8.0+24`
> **当前候选状态：** M4-B/F8 `CANDIDATE READY — DEVICE ACCEPTANCE PENDING`
> **项目阶段：** 私有自用；M3 已接受，M4-A、M4-B与拍摄时间/GPS增强处于真机验收阶段
...
**C1a已实现，但项目所有者明确跳过其真机验收；未执行矩阵仍是PENDING，不能写PASS。** D1已完成。M4-B/F8首个冻结SHA的一次双路复审为一路PASS、一路BLOCKERS；接受的`INVALID_TRIM`恢复锁定问题已在唯一纠正SHA `9351e75...`中修复，完整自动化和APK静态核验通过。按预算不追加第二轮复审，旧SHA裁决不得写成纠正SHA独立PASS。C1b/C2/C3/M4-C仍未授权。所有既有真机债务不会因新候选而自动转为PASS：
...
M4-B已按授权范围完成内部候选；M4-C、M5打磨、C1b/C2/C3、hardening、refactor、migration均**没有自动开工权限**。
...
| M4-B时间裁剪 | `CANDIDATE READY — DEVICE ACCEPTANCE PENDING` | 纠正SHA `9351e75...`通过Flutter `244/244`、Android JVM `346/346`与APK独立核验；多段/跨文件不在范围，真机证据尚未执行 |
...
1. M4-B单段时间裁剪真机验收（当前；实现和内部候选已完成）；
...
### 7.2 M4-B：单段时间裁剪 F8（r2补全计划；r10 `CANDIDATE READY — DEVICE ACCEPTANCE PENDING`）
...
3. M4-B真机验收未结束前，不启动C1b/C2/C3、M4-C、hardening、refactor或migration。
...
1. 是否已经覆盖安装`1.8.0+24`并执行`docs/m4-b-device-acceptance.md`；
2. M4-B实际起止段、音画同步、trim组合、后台/恢复/retry和文件安全证据，以及接受/拒绝决定；
3. 两个既有真机矩阵（M4-A十项、时间/GPS来源）的进展；C1a已由所有者跳过，不得要求补测或写PASS；
4. 只有M4-B明确接受后，才询问是否授权下一个独立项目；推荐顺序仍是C2，再依据证据决定C1b/C3或“都不做”；
