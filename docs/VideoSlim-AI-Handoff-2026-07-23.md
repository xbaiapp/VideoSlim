...
>
> **生成日期：** 2026-07-24
> **修订：** r15（2026-07-24）— 冻结同任务、同服务、无确认、最多一次的自动software decoder fallback；`1.9.1+26 / 8f4e970...`静态门禁与APK核验通过，真实device path PENDING。原C2/F21能力报告保持冻结。规划演进与实现记录见§16。
...
> **规范分支：** `m4a/crop`
> **当前可执行候选源码：** `8f4e970c8c724a5019bcc0f56cbbddbb47d2fb33`
> **当前候选版本：** `1.9.1+26`
> **当前候选状态：** 一次性自动软件解码重试 `PRIVATE INTERNAL CANDIDATE — DEVICE ACCEPTANCE PENDING`
> **当前已接受基线：** M4-B/F8 `1.8.0+24 / 9351e75...`（所有者报告成功；详细矩阵未提供）
> **项目阶段：** 私有自用；自动fallback候选等待Pixel真机验收；C2报告与原候选证据保持独立冻结；其他代码项仍需独立授权
...
**C1a已实现，但项目所有者明确跳过其真机验收；未执行矩阵仍是PENDING，不能写PASS。** D1已完成。M4-B/F8已由项目所有者报告成功并按private scope接受。C2/F21的`1.9.0+25 / 11f169c...`与Pixel 10 Pro/API 37能力报告保持冻结。自动fallback首版`11799b0...`通过门禁，但唯一独立exact-state review在600秒超时且无verdict；controller发现的延迟硬件snapshot attempt-boundary和registry fail-open已在唯一修订`8f4e970...`关闭。该修订通过Flutter`259/259`、Android JVM`359/359`、完整lint/build与APK静态核验，但没有独立review PASS，真实decoder failure真机路径仍PENDING。C1b/C3/M4-C仍未授权。所有既有真机债务不会因新候选而自动转为PASS：

1. 复测约1 MiB日志复制不再触发Android Binder异常；
2. 补齐M4-A裁剪真机矩阵；
3. 补齐真实iPhone/Pixel来源的时间/GPS、MediaStore `DATE_TAKEN`、图库排序和SAF矩阵；
4. 按自动fallback独立清单验证正常硬件回归；若故障自然重现，证明0%重置、唯一软件attempt、唯一发布和清理；
5. 由项目所有者决定接受、拒绝或授权一个明确的小修。
...
| 一次性自动软件解码重试 | `PRIVATE INTERNAL CANDIDATE — DEVICE ACCEPTANCE PENDING` | `1.9.1+26 / 8f4e970...`静态门禁和APK核验通过；同task/同service、无确认、最多一次；真实failure path尚待Pixel证据 |
...
versionName/versionCode：1.9.1 / 26
源码：8f4e970c8c724a5019bcc0f56cbbddbb47d2fb33
源码 tree：54059b52b5cb9a65419ab65e8815f4527946c3d3
ABI：arm64-v8a only
...
### 6.2 当前唯一优先队列：自动software decoder fallback真机验收

代码实现、唯一review/revision预算、纠正SHA门禁与APK静态核验均已完成。当前不再修改代码，只执行`docs/automatic-software-decoder-retry-device-acceptance.md`。

#### P0：覆盖安装并验证当前候选

1. 使用相同Debug certificate覆盖安装`1.9.1+26`，不要先卸载，以保留应用数据和F19日志；
2. 用已知同源/同crop/trim/参数先验证正常hardware成功路径，确认没有误触发fallback且只发布一个文件；
3. 若非确定性hardware decoder failure自然重现，核对无确认、同task、同service、`preparing/0%`、实际software decoder、encoder/请求参数不变且最多一次；
4. 验证成功、software failure与取消都不会循环，旧temp/recovery不泄漏，最终最多一个公共输出；
5. Activity后台/锁屏/重建后仍恢复同一个software attempt；保留完整F19日志、UI和前台通知截图。
...
真机证据后，项目所有者对`1.9.1+26`明确选择`ACCEPTED — private scope`、`REJECTED — blocker found`，或只授权一个真机可复现/文件安全问题的局部修订。未获得决定前不得把自动fallback写为ACCEPTED。
...
2. 一次性自动software decoder fallback（`1.9.1+26`候选已完成，当前只做真机验收）；
...
**当前最安全的继续方式：覆盖安装`1.9.1+26 / 8f4e970...`私有候选，按自动fallback清单先验证正常硬件路径；若非确定性decoder failure自然重现，回传完整F19日志及UI/通知证据。在新授权前不得继续修改生产代码或并行扩张到C1b/C3/M4-C及其他项目。**
