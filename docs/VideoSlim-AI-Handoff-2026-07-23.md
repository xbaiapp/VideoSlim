...
>
> **生成日期：** 2026-07-23
> **修订：** r7（2026-07-23）— 项目所有者跳过C1a真机验收并要求继续；D1使用此前提供的最新相关F19任务完成零代码诊断，M4-B/F8连续单段时间裁剪成为当前唯一获准代码项。规划演进与实现记录见§16。
...
**C1a已实现，但项目所有者明确跳过其真机验收；未执行矩阵仍是PENDING，不能写PASS。** D1已完成；M4-B/F8连续单段时间裁剪现为唯一获准代码项。C1b/C2/C3/M4-C仍未授权。当前`1.7.0+23`候选的既有真机债务不会因继续开发而自动转为PASS：
...
| M4-B时间裁剪 | `AUTHORIZED — PLANNING` | F8连续单段trim是当前唯一代码项；多段/跨文件不在范围 |
...
### 7.2 M4-B：单段时间裁剪 F8（r2补全计划；r7 `AUTHORIZED — PLANNING`）
