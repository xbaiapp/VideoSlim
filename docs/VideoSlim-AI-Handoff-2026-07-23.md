...
>
> **生成日期：** 2026-07-23
> **修订：** r5（2026-07-23）— r4 吸收 Codex 一轮仓库实证复核；r5 按其二轮复核做小幅收敛：清理旧措辞、废弃 3% 推断改为显式契约、C1 拆分为 C1a/C1b 并与 D1 结果闭环、建议值合法范围与 codec 白名单、C1a 选项按 crop 条件显示、C3c 去除预设硬件结论；变更清单见 §16。历次修订均未改动任何代码。
...
> **当前候选版本：** `1.6.1+22`
> **C1a开发候选版本：** `1.7.0+23`（唯一修订中，尚未构建或替代当前可安装候选）
...
**当前唯一实施项是C1a低收益/可能变大提示。** C1b/C2/C3/M4-B/M4-C仍未授权。`1.6.1+22`仍是当前可安装候选，其既有真机验收债务不会因C1a开发而自动转为PASS：
...
r2 补充：项目所有者已于 2026-07-23 指示把两条新需求写入规划——**压缩策略修正（§7.1 C 轨：应对低码率源越压越大）**与**时间编辑（§7.2 M4-B 单段、§7.3 M4-C 多段）**。它们目前状态均为 `PLANNED — NOT AUTHORIZED`：计划已成文，实施仍需所有者按 §7.0 排期逐项授权。
...
| M4-B时间裁剪 | `PLANNED — NOT AUTHORIZED` | r2 起已有书面计划（§7.2，由所有者指示写入）；实施仍需明确授权 |
| M4-C多段时间编辑 | `PLANNED — NOT AUTHORIZED` | r2 新增规划（§7.3）；依赖 M4-B 验收通过 |
| 压缩策略修正 C 轨（D1/C1a/C1b/C2/C3） | `C1a AUTHORIZED — REVISION IN PROGRESS` | C1a是唯一代码项；D1待指定日志，C1b/C2/C3仍未授权 |
...
r4 基线补充（来源：所有者侧 AI 仓库走读，2026-07-23）：当前 HEAD 为 `9796b0da46786f7d63c371c34b30e0caa53639ac`，与 `origin/m4a/crop` 同步、工作区干净；自 `b0267a0` 起 `android/`、`lib/`、`test/` 无代码变化、仅文档变化，故当前 APK 对应的生产源码仍为 `b0267a0`。
...
1. 完成已明确选择的C1a低收益/可能变大提示（小，纯Dart）；它不替代§6挂起的P0/P1真机验收；
2. 完成§6挂起的P0/P1真机验收，并按指定任务读取既有F19记录执行D1零代码诊断；仅当记录缺失时才另行授权一条日志的最小修；
3. C1b建议目标为后置条件项（依赖显式码率来源契约、D1结论与系数校准，见§7.1），与C3决策窗口一并评估，不单列排期；
4. M4-B 单段时间裁剪（中）；
5. C2 编码能力探测（小型诊断改动，不改转码行为，可与 M4-B 前后穿插）；
6. C3 按 C2 证据四选一，含"都不做"（中）；
7. M4-C 多段时间编辑（中大，依赖 M4-B 验收）；
8. M5 原范围顺延，其中 F13 建议提级（见 7.4 r2 备注）。

每项独立授权、独立验收；同一时间不并行两条编码任务。r4 修正：D1 第一步（读既有日志）零代码、可立即执行，无需授权流程；C2 虽只查询能力、不改转码行为，但仍是完整的小型代码任务（候选、测试、复审、真机），按独立小候选走，不与 D1 捆绑。

### 7.1 C 轨：压缩策略修正（C1a `AUTHORIZED — REVISION IN PROGRESS`；其余未授权）
...
- 剩余问题：Media3 fallback 后的有效 encoder configuration `Format` 中记录的目标码率是多少——是 fallback 在**配置期**把值夹高/换了编码器，还是有效配置仍约为 500k 而编码器**运行期**明显过冲。现有 `codec.configurationFormat` 是 `androidx.media3.common.Format`，不是原始 `android.media.MediaFormat`；若以后必须证明传给 `MediaCodec.configure()` 的 `KEY_BIT_RATE`，需另加底层证据。
- 步骤（第一步零代码）：
  1. 直接读取 §8 那次任务的 F19 日志中 `actual video encoder name=... format=...` 一行，取其中的码率字段与编码器名；
  2. 仅当该行缺失或不含最终有效码率时，才授权最小日志修订（一次实现、一次修订、一轮复审），并用同素材复测。
- 结果分支：
...
#### C1 源感知提示（C1a，已授权）与建议性目标（C1b，未授权）（r5 拆分）
...
### 7.2 M4-B：单段时间裁剪 F8（r2 补全计划，`PLANNED — NOT AUTHORIZED`）

计划由所有者于 2026-07-23 指示写入；实施仍需另行明确授权。
...
r2 备注（r5 更新）：4.6 倍的目标偏差本身值得诊断（§7.1 D1）。r4 代码证据已排除"请求未接线/被 HQ targeting 覆盖"，剩余需要区分的是"Media3 fallback 在配置期把目标夹高"与"配置约 500 kbps 后硬件编码器运行期明显过冲"——读取该次任务 F19 中既有的 encoder configurationFormat 记录即可判定。诊断是查明事实，不等于改变已确认的 VBR 政策。
...
1. 是否已经覆盖安装并复测 `1.6.1+22` 的超长日志复制（P0）；
2. P1 两个真机矩阵（M4-A 十项、时间/GPS 来源）的进展，以及是否把当前候选接受为 private scope；
3. 是否提供或指定该次任务的 F19 日志用于 D1 只读诊断（读取既有记录无需授权）；仅当日志字段不足时，再另行询问是否授权日志小修；
4. C 轨（C1a → C2 → C1b/C3 决策）与 M4-B 谁先排期（推荐顺序见 §7.0）；
