...
**当前范围说明**：M2 的“自定义”是指用户可修改上述编码参数；视频压缩量以**目标平均码率（CBR）**控制，并在发布前校验输出没有超过显示的保守上界。当前规格未包含“画质百分比/恒定质量（CQ/CRF）”“CBR/VBR 模式切换”或“按原文件百分比压缩”。“按目标文件大小压缩”已列为 F11，计划在 M5 实现。
