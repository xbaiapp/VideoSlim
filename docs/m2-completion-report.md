...
> **状态（2026-07-20）：** `1.3.5+14` 的软件输入解码兼容路径、准确错误语义、保存位置/体积区间、SAF 发布恢复和实际编码模式已完成本地自动化与 APK 静态核验。Pixel 10 Pro / Android 17 A/B/C 和软件兼容模式尚未真机执行，因此当前仅为**私有验收候选**，不得称为生产发布完成或进入 M3。
