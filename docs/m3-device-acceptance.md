...
> **状态：** 无已连接/授权的 adb 设备，所有 Pixel/硬件行保持未执行、未认领。非真机门禁（源码测试、独立复审、APK 静态核验和源码溯源）按精确 commit SHA 记录在 `/root/artifacts/videoslim/m3/candidates/<full-sha>/release-checklist.md`；不得用这些证据代替本表真机结果。
