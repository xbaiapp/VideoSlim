...
> **更新时间：** 2026-07-23
> **当前阶段：** M4-B/F8单段时间裁剪 `CORRECTIVE REVISION — EXACT-SHA GATES PENDING`
...
| M4-B 时间裁剪 | `CORRECTIVE REVISION — EXACT-SHA GATES PENDING` | 首个冻结SHA的一次双路复审发现一项`INVALID_TRIM`恢复阻断；唯一纠正修订已通过Flutter `244/244`，待冻结后完整门禁和APK核验 |
...
M4-B首个冻结SHA `9c9ca887ec4a8a26a6eb892077f88d333eb2a0da`通过Dart format、Flutter analyze、Flutter tests `241/241`、Android JVM tests `346/346`、debug/release lint和debug assemble，但其一次双路复审为一路PASS、一路BLOCKERS。质量审查发现终态`INVALID_TRIM`快照在来源metadata缩短时会先抛错并进入所有权不确定锁定，阻断用户重新编辑/移除。

该旧SHA已否决且不得构建候选。唯一纠正修订保留快照端点、显式标记`trimNeedsRepair`，并以“不夹取、不自动保存、用户重新选择或移除”的编辑器状态恢复。RED三项按预期失败后，纠正工作树已通过Dart format、Flutter analyze（0 issues）和Flutter tests `244/244`。按每任务一轮复审预算不发起第二轮，也不得把旧SHA的混合裁决写成纠正SHA的PASS；完整处置见`docs/m4-b-exact-sha-review-disposition.md`。纠正提交仍需完整exact-SHA门禁和APK独立核验，这些自动化结果不是APK或真机PASS。
...
6. C1b/C2/C3、M4-C/F23、M5历史/批量/目标大小和M6 iOS/上架均未开始；M4-B虽已实现并用掉唯一复审/修订预算，但纠正SHA尚未完成exact-SHA门禁、APK静态核验或真机验收。
...
下一步是冻结M4-B纠正源码SHA、在该提交上复跑完整门禁并构建/独立核验`1.8.0+24` ARM64内部候选；唯一双路复审和唯一修订预算均已使用，不得自动追加第二轮。C1a与既有真机矩阵保留为未测试债务，不得改写为PASS。
...
4. `docs/m4-b-exact-sha-review-disposition.md`
5. `docs/m4-b-device-acceptance.md`
6. `docs/c1a-low-savings-completion-report.md`
7. `docs/d1-bitrate-diagnosis-2026-07-23.md`
8. `docs/c1a-low-savings-device-acceptance.md`
