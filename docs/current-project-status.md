...
> **更新时间：** 2026-07-24
> **当前阶段：** 一次性自动软件解码重试 `PRIVATE INTERNAL CANDIDATE — DEVICE ACCEPTANCE PENDING`
...
| 一次性自动软件解码重试 | `PRIVATE INTERNAL CANDIDATE — DEVICE ACCEPTANCE PENDING` | `1.9.1+26 / 8f4e970...`静态门禁、ARM64 build与APK核验通过；同任务、同服务、无确认、最多一次；真实硬件decoder failure路径尚待真机证据 |
...
- 版本：`1.9.1+26`
- 候选源代码：`8f4e970c8c724a5019bcc0f56cbbddbb47d2fb33`
- 候选 tree：`54059b52b5cb9a65419ab65e8815f4527946c3d3`
...
- APK：`VideoSlim-1.9.1+26-8f4e970-arm64-v8a-release.apk`（仅供私有真机验收）
- APK 大小：`18,509,915` bytes
- APK SHA-256：`8765c79b1afe9cfe5b451359766646b4be089d12820e3478a05ba636f24272f3`

`1.9.1+26`现在是最新可执行候选；M4-B `1.8.0+24 / 9351e75...`仍是项目所有者已接受的私有基线。原C2 `1.9.0+25 / 11f169c...`及API 37完整能力清单保持冻结，不被自动retry候选覆盖；系统声明仍不能升级为production签名、实际编码成功或QP/CQ/AV1实现授权。
...
自动软件decoder fallback的新候选尚未完成真机验收。正常硬件成功路径必须先回归；非确定性硬件failure若自然重现，必须证明同task无确认切到software、进度/通知回到0%、最多一次、最终最多发布一个文件且无temp/recovery泄漏，详见`docs/automatic-software-decoder-retry-device-acceptance.md`。C2已在Pixel 10 Pro / Android 17 API 37上返回完整确定性清单，复制回传PASS；刷新、返回后设置不变、查询无任务/通知/文件副作用、重启重查和并发普通压缩仍未获得独立设备证据。项目所有者已报告M4-B测试成功并接受private scope；由于没有提供完整设备/素材/PTS/截图矩阵，`docs/m4-b-device-acceptance.md`未报告的行继续保持`PENDING`。`docs/c1a-low-savings-device-acceptance.md`已由所有者明确跳过且所有行仍为`PENDING`；`docs/capture-metadata-device-acceptance.md`和独立M4-A裁剪矩阵也保持`PENDING`，包括：
...
6. C2/F21证据仍缺完整交互/副作用矩阵；一次性自动software fallback的唯一独立review为超时无裁决，纠正SHA没有独立review PASS，且非确定性硬件failure的真实device path尚未复现。
7. C1b/C3、M4-C/F23、M5历史/批量/目标大小和M6 iOS/上架均未开始。

## 7. 已批准的规划基线

- `docs/VideoSlim-AI-Handoff-2026-07-23.md` 已整合项目所有者批准的C轨和时间编辑规划；
- `docs/VideoSlim PRD.md` v1.21 在保留C轨/F23边界的同时，冻结同任务、同服务、无确认、最多一次的软件decoder fallback contract；
...
下一步是项目所有者按`docs/automatic-software-decoder-retry-device-acceptance.md`安装并验证`1.9.1+26`：先确认正常硬件成功路径；若非确定性decoder failure自然重现，再核对自动software attempt、0%重置、唯一发布与清理。C2报告处置和C3选择继续保持独立，不因本候选自动启动；任何C3方向仍需新的明确授权与真实编码证据。
...
4. `docs/automatic-software-decoder-retry-completion-report.md`
5. `docs/automatic-software-decoder-retry-device-acceptance.md`
6. `docs/c2-encoder-capabilities-completion-report.md`
7. `docs/c2-exact-sha-review-disposition.md`
8. `docs/c2-encoder-capabilities-device-acceptance.md`
9. `docs/plans/2026-07-23-c2-encoder-capabilities.md`
10. `docs/m4-b-completion-report.md`
11. `docs/m4-b-exact-sha-review-disposition.md`
12. `docs/m4-b-device-acceptance.md`
13. `docs/c1a-low-savings-completion-report.md`
14. `docs/d1-bitrate-diagnosis-2026-07-23.md`
15. `docs/c1a-low-savings-device-acceptance.md`
16. `docs/capture-metadata-completion-report.md`
17. `docs/capture-metadata-device-acceptance.md`
18. `docs/m4-a-completion-report.md`
19. `docs/m4-device-acceptance.md`
20. `docs/VideoSlim PRD.md`
21. `docs/known-debt.md`
22. `AGENTS.md`
