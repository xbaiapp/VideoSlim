...
> **更新时间：** 2026-07-24
> **当前阶段：** `1.9.2+27` AAC 无损 copy cadence 修正已取得 App 端真机成功日志，主观播放确认 PENDING；一次性自动 software decoder fallback 的自然 failure contract 已在 Pixel 设备复现，所有者处置 PENDING
> **当前可执行候选：** `1.9.2+27 / 7948f9fdbfc839b26d6055ca83ddf8449df42de5`
> **当前已接受私有基线：** M4-B `1.8.0+24 / 9351e75bcc43c71a6e7caf03093fe27b0072b061`（所有者报告测试成功；详细矩阵未提供）
> **上一已接受音频基线：** M3 `1.4.3+18 / 19abfb7da2e8fa028e7200000f0dc2a114bc840e`；本次 `1.9.2+27` cadence 修正仍等待新输出的主观播放确认
...
| M3 音频提取 | `ACCEPTED — private scope` | 项目所有者于 2026-07-22 接受原 M3 私有范围；未提供逐项矩阵不补写 PASS |
| M3 AAC copy cadence 修正 | `APP-LEVEL DEVICE LOG PASS — SUBJECTIVE PLAYBACK PENDING` | `1.9.2+27 / 7948f9f...`正确接受真实 `23,812 µs` 小抖动，完整 frame gap 仍失败；同类素材已单次发布成功，实际播放/声音待确认 |
...
| 一次性自动软件解码重试 | `NATURAL DEVICE FALLBACK CONTRACT OBSERVED — OWNER DISPOSITION PENDING` | 首次硬件解码在约79%结构化失败后，同task/同service无确认切到软件、回到0%、仅一次；software attempt后续死于独立硬件Encoder错误，没有循环、残缺发布或recovery泄漏 |
...
`ACCEPTED — private scope` 只表示项目所有者接受已测试的自用设备/素材范围，不表示生产发布或多设备保证。`CANDIDATE READY`、`DEVICE LOG PASS` 或自然 fallback contract 已观察，也都不等于所有主观/设备矩阵已通过。
...
- 版本：`1.9.2+27`
- 可执行候选源代码：`7948f9fdbfc839b26d6055ca83ddf8449df42de5`
- 可执行候选 tree：`a668c12b9f9dca7d6ea5024c55343850d83a661b`
- 后续 workflow-only 修正：`68aaa777ef39b1b71441dbc9da3d17ce78258301`；没有生产代码或 APK delta，不取代上述可执行身份
...
- APK：`VideoSlim-1.9.2+27-7948f9f-arm64-v8a-release.apk`（仅供私有真机验收）
- APK 大小：`18,509,915` bytes
- APK SHA-256：`7a180bc7ce8f89439c7e31f1fcb4c019d1be45d4d356c436698aad43bbcb6f18`

`1.9.2+27`现在是最新可执行候选；它保留`1.9.1+26 / 8f4e970...`的一次性自动software decoder retry实现，只修正M3 AAC无损copy cadence。M4-B `1.8.0+24 / 9351e75...`仍是项目所有者已接受的私有基线。原C2 `1.9.0+25 / 11f169c...`及API 37能力清单保持冻结；系统声明不能升级为production签名、实际高级编码成功或QP/CQ/AV1实现授权。

M3已接受APK、旧M4-A/日志修复候选及各自证据继续保留，不被新候选覆盖或改写。初版 cadence SHA `fe0e04d...`因缺少绝对小抖动上限已撤销并隔离，不得交付。
...
- 首次硬件视频attempt若返回结构化`VIDEO_DECODING_FAILED`，native在同一用户任务与同一`ProcessingService`内自动从头用software decoder重试一次；精确复用request并只改decoder mode，route/publication/registry/recovery转移后旧route迟到事件失效；不确认、不循环、不永久降级硬件；
...
一次性自动软件解码重试首个实现SHA `11799b06dec490d4cab4c8094b10377580c5971b`通过当时的静态门禁，但唯一独立exact-state review在600秒超时、29次只读调用后无summary，按`NO VERDICT`处理。controller随后发现延迟硬件snapshot可能压过software attempt的0%重置，以及registry转移失败后仍best-effort继续；两项均在唯一修订SHA `8f4e970c8c724a5019bcc0f56cbbddbb47d2fb33`中关闭。纠正SHA exact证据：Dart format `63/0`、Flutter analyze 0 issues、Flutter `259/259`、Android JVM `359/359`（58个XML、0 failures/errors/skipped）、debug/release lint与assemble、ARM64 release build和APK静态核验全部PASS。按一轮review预算不宣称纠正SHA有独立review PASS。完整证据见`docs/automatic-software-decoder-retry-completion-report.md`。

M3 AAC lossless-copy cadence 修正先以真实 `805 samples / 23,812 µs` fixture建立RED，再在仅限copy的source-relative规则上转GREEN。初版冻结SHA `fe0e04d...`的一次双路只读审查为integration PASS、correctness BLOCKER：source/output共同拥有完整AAC frame gap时可能错误放行。唯一修订SHA `7948f9fdbfc839b26d6055ca83ddf8449df42de5`加入`MAX_INHERITED_SOURCE_JITTER_US = 1,000 µs`绝对上限和对应RED→GREEN测试；最终focused `22/22`、Flutter `259/259`、Android JVM `365/365`、完整lint/build及APK核验PASS。初版APK已撤销。完整证据见`docs/m3-lossless-copy-cadence-correction-completion-report.md`。
...
自动software decoder fallback的非确定性真实failure path已自然复现：首次硬件Decoder在约79%结构化失败，同task/同service无确认切到software并回到0%，实际软件Decoder启动；后续独立硬件Encoder失败正确终止，没有循环、残缺发布或recovery泄漏。自动fallback最终成功发布/播放、通知截图、software取消和后台恢复仍PENDING。M3 cadence修正后同类AAC素材已在App端单次发布成功；人耳播放/声音仍PENDING。C2已在Pixel 10 Pro / Android 17 API 37上返回完整确定性清单，复制回传PASS；刷新、返回后设置不变、查询无任务/通知/文件副作用、重启重查和并发普通压缩仍未获得独立设备证据。项目所有者已报告M4-B测试成功并接受private scope；由于没有提供完整设备/素材/PTS/截图矩阵，`docs/m4-b-device-acceptance.md`未报告的行继续保持`PENDING`。`docs/c1a-low-savings-device-acceptance.md`已由所有者明确跳过且所有行仍为`PENDING`；`docs/capture-metadata-device-acceptance.md`和独立M4-A裁剪矩阵也保持`PENDING`，包括：
...
2. API 35 x86_64 instrumentation的历史`sdkmanager` PATH问题已由workflow-only修正`68aaa77...`关闭；远端run`30107559081`主门禁全PASS，API 35 x86_64 instrumentation `3/3 PASS`。这不是物理设备验收替代品。
...
6. C2/F21证据仍缺完整交互/副作用矩阵；一次性自动software fallback的纠正SHA没有独立review PASS，真实contract已观察但最终成功发布/播放等矩阵仍PENDING；M3 cadence修正的主观播放确认仍PENDING。
7. C1b/C3、M4-C/F23、M5历史/批量/目标大小和M6 iOS/上架均未开始。

## 7. 已批准的规划基线

- `docs/VideoSlim-AI-Handoff-2026-07-23.md` 已整合项目所有者批准的C轨和时间编辑规划；
- `docs/VideoSlim PRD.md` 在保留C轨/F23边界的同时，冻结同任务、同服务、无确认、最多一次的软件decoder fallback contract以及M3 lossless-copy窄cadence继承边界；
...
当前可继续且不需要生产范围决策的工作只剩冻结文档证据，并保留M3新输出的主观播放确认以及自动fallback最终成功/通知/取消/后台矩阵为设备PENDING。workflow-only CI重跑已经全绿。其后生产路线到达明确决策点：C1b/C3/F22与M4-C/F23均未获授权，不能从“继续”泛化为任选一个生产功能。C2报告处置和任何C3方向仍需项目所有者明确选择与真实编码证据。
...
4. `docs/m3-lossless-copy-cadence-correction-completion-report.md`
5. `docs/m3-device-acceptance.md`
6. `docs/ci-api35-instrumentation-recovery-2026-07-24.md`
7. `docs/automatic-software-decoder-retry-completion-report.md`
8. `docs/automatic-software-decoder-retry-device-acceptance.md`
9. `docs/c2-encoder-capabilities-completion-report.md`
10. `docs/c2-exact-sha-review-disposition.md`
11. `docs/c2-encoder-capabilities-device-acceptance.md`
12. `docs/plans/2026-07-23-c2-encoder-capabilities.md`
13. `docs/m4-b-completion-report.md`
14. `docs/m4-b-exact-sha-review-disposition.md`
15. `docs/m4-b-device-acceptance.md`
16. `docs/c1a-low-savings-completion-report.md`
17. `docs/d1-bitrate-diagnosis-2026-07-23.md`
18. `docs/c1a-low-savings-device-acceptance.md`
19. `docs/capture-metadata-completion-report.md`
20. `docs/capture-metadata-device-acceptance.md`
21. `docs/m4-a-completion-report.md`
22. `docs/m4-device-acceptance.md`
23. `docs/VideoSlim PRD.md`
24. `docs/known-debt.md`
25. `AGENTS.md`
