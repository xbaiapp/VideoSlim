...
> **当前状态（2026-07-23）：** M3 `1.4.3+18` 仍是已接受私有基线。当前 `1.6.1+22` 保留已复审的拍摄时间/GPS修订，并修复设备发现的约1 MiB日志复制Binder超限；一条Pixel设备“时间存在、位置缺失”任务已成功，完整真实来源/图库/SAF验收仍待执行。M4-B时间裁剪仍未授权。完整进度见 `docs/current-project-status.md`。
...
当前版本不包含 MP3、多音轨选择、trim、混音、降噪、采样率选择、隐私模式或完整metadata复制。
...
当前候选的真实来源/设备矩阵见 `docs/capture-metadata-device-acceptance.md`，裁剪矩阵见 `docs/m4-device-acceptance.md`。M3测试素材可用 `tool/generate_m3_fixtures.sh` 生成；二进制fixture不提交到Git。
